...
class LSTM_model(nn.Module):
	...
	self.attention = nn.MultiheadAttention(embedding_dim,input_size)
	...
	out,_ = self.attention(x,x,x)
        output_lstm, states_lstm = self.lstm1(out, self.zero_hidden())
...
def train_loop(model, num_epochs, CE_loss, optimizer, X,y):
    ...
    for x in range(num_epochs):
		...
		min_loss = loss.item()
		torch.save(LSTM_model.state_dict(),'language_generator_att_min.ckpt')
...
torch.save(LSTM_model.state_dict(),'language_generator_att.ckpt')
